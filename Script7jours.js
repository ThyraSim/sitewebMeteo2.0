...
fetch('temperatures_2023.json')
    ...
    function Septjour1(liste, newDate)
    {
        for (var i = 1; i <= 7; i++) {
            var Septicone = document.createElement("img");
            var Septjour = document.getElementById("Septjour" + i);
            var Septdate = document.getElementById("Septdate" + i);
            var SepttempAjourdhui = document.getElementById("Septtemps" + i);
            var SepttempMin = document.getElementById("SepttempsMin" + i);
            var SepttempMax = document.getElementById("SepttempsMax" + i);
            for (let index = 0; index < liste.length; index++) {
                ...
                {
                    dayOfWeek(dateJSON)
                    Septjour.innerHTML = jourSemaine
                    mois(dateJSON)
                    Septdate.innerHTML = moisChoisi
                    SepttempAjourdhui.innerHTML = temp.TempDuJour + "&deg;C"
                    SepttempMin.innerHTML = "MIN\n" + temp.TempMin + "&deg;C"
                    SepttempMax.innerHTML = "MAX\n" + temp.TempMax + "&deg;C"
                    chooseIcon(temp.TempDuJour)
                    Septicone = document.getElementById("Septicone"+i)
                    Septicone.src = icone
                    Septicone.width = "100"
